...
from
    v_users_resources
WHERE
    owner_mailing = 'redactor';
select 
    *
from
    v_users_resources
WHERE
    owner_mailing = 'revisor';
select 
    *
from
    v_users_resources
...
from
    v_sourcetarget
where
    source like 'tuna%' and solang = 'EN';
select 
    *
from
    v_sourcetarget
where
    source like 'sand%' and solang = 'EN';
select 
    *
from
    v_sourcetarget
where
    source like 'CESAP' and solang = 'FR';
select 
    *
from
    v_sourcetarget
...
from
    v_sourcetarget
where
    id_resource in (1) and solang = 'EN'
        and talang = 'FR';
...
create or replace view v_editedbyowner as
    select distinct
        id_concept, status, lastmodified_by
    from
        v_source;
...
from
    v_editedbyowner
where
    status = 'e' and lastmodified_by = 1070;

create or replace view v_getformsbyconcept as
    select 
        c.id_concept, t.source, t.solang
    from
        concepts c,
        v_source t
    where
        c.id_concept = t.id_concept;
...
create or replace view v_getformsbyconcept as
    select 
        c.id_concept, c.id_language, ifnull(t.source, '?') source
    from
        v_conceptprodlang c
            left join
        v_source t ON (c.id_concept = t.id_concept
            and c.id_language = t.solang);


create or replace view vj_getformsbyconcept as
    SELECT 
        uuid() uuid, v_getformsbyconcept . *
    FROM
        v_getformsbyconcept;
...
from
    v_source
where
    status = 'r' and solang IN ('EN')
        and id_resource in (1);
...
create or replace view v_source_lang as
    select distinct
        t1.id_language solang, c.id_concept
    from
        terms t1,
        langsets l1,
        concepts c
...
create or replace view v_users_languages as
    select 
        ...
        o.owner_mailing,
        l.id_language,
        l.language_default_name
    from
        languages l,
        owners o,
        users_languages ul
    where
        l.id_language = ul.id_language
...
create or replace view vj_users_languages as
    SELECT 
        uuid() uuid, v_users_languages . *
    FROM
        v_users_languages;


select 
    resource_name, id_language, count(*) nbterms
from
    v_reslang
group by resource_name , id_language;
