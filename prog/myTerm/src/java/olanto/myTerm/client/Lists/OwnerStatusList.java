...
public class OwnerStatusList extends ListBox {
    ...
    private AsyncCallback<Map<String, String>> ownerStatusCallback;


    public OwnerStatusList(String langID) {
        super();
        ownerStatusCallback = new AsyncCallback<Map<String, String>>() {
            @Override
    ...
    public OwnerStatusList(String langID, final String currentStatus, final BooleanWrap isEdited, final BooleanWrap isLocallyEdited) {
        super();
        ownerStatusCallback = new AsyncCallback<Map<String, String>>() {
            @Override
            public void onFailure(Throwable caught) {
            ...
            public void onSuccess(Map<String, String> result) {
                ...
                if (res.addAll(result.keySet())) {
                    ...
                    for (String s : res) {
                        ...
                        if (result.get(s).equalsIgnoreCase(currentStatus)) {
                            i = res.indexOf(s);
                        }
                    }
                    setSelectedIndex(i);
